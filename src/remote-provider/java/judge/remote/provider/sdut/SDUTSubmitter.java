...
public class SDUTSubmitter extends CanonicalSubmitter {
    ...
    protected Integer getMaxRunId(SubmissionInfo info, DedicatedHttpClient client, boolean submitted) {
        ...
        Matcher matcher = Pattern.compile("<tr data-spj=[\\s\\S]+?(>|class=[\\S]+?>)[\\s]+?(<input[\\w]+?>|)[\\s]+?<td>(\\d+?)</td>[\\s\\S]+?<td class=\"nowrap-td\">").matcher(html);
        return matcher.find() ? Integer.parseInt(matcher.group(0).replaceAll("<.*?>", "").trim()) : -1;
    }
